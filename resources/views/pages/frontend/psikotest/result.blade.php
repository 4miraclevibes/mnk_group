...
<body>
    <div class="result-container">
        ...
        @php
            // Extract data from description untuk kecermatan
            ...
            // Extract hanya bagian deskripsi utama
            $mainDescription = preg_replace('/\s*\(.*\)/', '', $result->description);
        @endphp

        <div class="score-circle {{ $calculatedScore >= 80 ? 'excellent' : ($calculatedScore >= 60 ? 'good' : ($calculatedScore >= 40 ? 'average' : 'poor')) }}">
            <div>
                <div style="font-size: 56px;">{{ number_format($calculatedScore, 1, ',', '.') }}</div>
            </div>
        </div>

        <div class="result-badge {{ $calculatedScore >= 80 ? 'excellent' : ($calculatedScore >= 60 ? 'good' : ($calculatedScore >= 40 ? 'average' : 'poor')) }}">
            {{ $mainDescription }}
        </div>
        ...
        <div class="exam-info">
            ...
            @php
                // Extract skor mentah dari description
                preg_match('/Skor: (-?\d+)/', $result->description, $scoreMatch);
                $rawScore = $scoreMatch[1] ?? 0;
                $totalAnswered = $correctAnswers + $wrongAnswers;
                $totalPossible = 500; // Total soal kecermatan adalah 500
            @endphp

            <div class="info-item">
                <span class="info-label">Skor Mentah:</span>
                <span class="info-value">{{ $rawScore }}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Total Dijawab:</span>
                <span class="info-value">{{ $totalAnswered }} / {{ $totalPossible }} soal</span>
            </div>
            <div class="info-item">
                <span class="info-label">Jawaban Benar:</span>
                <span class="info-value text-success">{{ $correctAnswers }} soal (+{{ $correctAnswers }})</span>
            </div>
            <div class="info-item">
                <span class="info-label">Jawaban Salah:</span>
                <span class="info-value text-danger">{{ $wrongAnswers }} soal (-{{ $wrongAnswers }})</span>
            </div>
            <div class="info-item">
        ...
        <div class="action-buttons">
            ...
            <a href="{{ route('kecermatan.history') }}" class="btn btn-primary-custom btn-custom">
